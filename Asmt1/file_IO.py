# File I/O functions for handling HTML and CSV dataset files
...
def load_from_csv(filename: str) -> list[dict]:
    ...
    with open(filename, 'r') as file:
        ...
        # newline character for CSV instead of </thead> for HTML
        #head, body = contents.split('\n') # doesn't work; no special head/body tags in CSV
        list_contents = contents.split('\n')
        head = list_contents[0] # first line is the header
        body_list_of_rows = list_contents[1:] # rest of the lines are the body

        # process the head first, pull out the column names
        head_parts = head.split(',')

        # iterate through non-header rows to create list of column names
        # not yet edited from HTML!
        columns = []
        for column_name in head_parts: #was [1:]
        ...
        for row_text in body_list_of_rows:
            values_list = row_text.split(',')
            ...
            for i in range(len(columns)):
                this_column = columns[i]
                this_value = values_list[i].strip()
